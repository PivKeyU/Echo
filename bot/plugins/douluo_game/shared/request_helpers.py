...
from bot.sql_helper.sql_douluo import upsert_profile_identity
...
def _upsert_douluo_profile_identity(user: dict[str, Any]) -> None:
    display_name = " ".join(
...
def verify_user_from_init_data(init_data: str) -> dict[str, Any]:
    return verify_telegram_user(init_data, on_verified=_upsert_douluo_profile_identity)
...
def verify_user_from_auth(init_data: str = "", session_token: str = "") -> dict[str, Any]:
    ...
    if tg <= 0:
        ...
        raise HTTPException(status_code=403, detail="完成 Telegram 绑定后即可进入斗罗大陆。")
    if raw_init:
    ...
    _upsert_douluo_profile_identity(user)
    return user


async def resolve_actor_from_request(request: Request) -> dict[str, Any]:
    body = await request.json()
    init_data = str(body.get("init_data") or "").strip()
    session_token = str(body.get("session_token") or "").strip()
    return verify_user_from_auth(init_data, session_token)
